...
class WPcom_JS_Concat extends WP_Scripts {
	...
	function do_items( $handles = false, $group = false ) {
		...
		foreach( $this->to_do as $key => $handle ) {
			...
			if ( ! $is_internal_url ) {
				do_action( 'qm/debug', sprintf( '[js concat] Skipped: %s. Reason: not internal URL', $js_url ) );
				$do_concat = false;
			}
			...
			if ( ! $js_realpath || 0 !== strpos( $js_realpath, ABSPATH ) ) {
				do_action( 'qm/debug', sprintf( '[js concat] Skipped: %s. Reason: outside ABSPATH', $js_url ) );
				$do_concat = false;
			} else {
			...
			if ( $this->has_inline_content( $handle ) ) {
				do_action( 'qm/debug', sprintf( '[js concat] Skipped: %s. Reason: has inline content', $js_url ) );
				$do_concat = false;
			}
			...
			if ( $do_concat && ! $do_concat_after ) {
				do_action( 'qm/debug', sprintf( '[js concat] Skipped: %s. Reason: filtered', $js_url ) );
			}
		...
		foreach ( $javascripts as $js_array ) {
			if ( 'do_item' == $js_array['type'] ) {
				...
				if ( isset( $js_array['paths'] ) && count( $js_array['paths'] ) > 1) {
					...
					$count = count( $paths );
					if ( $count > 150 ) {
						do_action( 'qm/debug', sprintf( '[js concat] TOO LARGE Bundle %s contains %d items: %s', $path_str, $count, "\n" . implode("\n", $paths ) ) );
					} else {
						do_action( 'qm/debug', sprintf( '[js concat] Bundle %s contains %d items: %s', $path_str, $count, "\n" . implode("\n", $paths ) ) );
					}

					$href = $siteurl . "/_static/??" . $path_str;
				} elseif ( isset( $js_array['paths'] ) && is_array( $js_array['paths'] ) ) {
					do_action( 'qm/debug', sprintf( '[js concat] Skipped: %s. Reason: lonely (nothing to concatenate with)', implode( $js_array['paths'] ) ) );
