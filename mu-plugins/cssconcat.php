	...
	function do_items( $handles = false, $group = false ) {
		...
		foreach( $this->to_do as $key => $handle ) {
			...
			if ( isset( $extra['conditional'] ) ) {
				do_action( 'qm/debug', sprintf( '[css concat] Skipped: %s. Reason: is conditional', $css_url ) );
				$do_concat = false;
			}

			// Don't concat rtl stuff for now until concat supports it correctly
			if ( 'rtl' === $this->text_direction && ! empty( $extra['rtl'] ) ) {
				do_action( 'qm/debug', sprintf( '[css concat] Skipped: %s. Reason: is rtl', $css_url ) );
				$do_concat = false;
			}
			...
			if ( ! $is_internal_url ) {
				do_action( 'qm/debug', sprintf( '[css concat] Skipped: %s. Reason: not internal url', $css_url ) );
				$do_concat = false;
			}
			...
			if ( ! $css_realpath || 0 !== strpos( $css_realpath, ABSPATH ) ) {
				if ( $css_url ) {
					...
					do_action( 'qm/debug', sprintf( '[css concat] Skipped: %s. Reason: outside ABSPATH', $css_url ) );
				}
				$do_concat = false;
			} else {
			...
			if ( $do_concat && ! $do_concat_after ) {
				do_action( 'qm/debug', sprintf( '[css concat] Skipped: %s. Reason: filtered', $css_url ) );
			}
		...
		foreach( $stylesheets as $idx => $stylesheets_group ) {
			foreach( $stylesheets_group as $media => $css ) {
				if ( 'noconcat' == $media ) {
					...
					$count = count( $paths );
					if ( $count > 150 ) {
						do_action( 'qm/debug', sprintf( '[css concat] TOO LARGE Bundle %s contains %d items: %s', $path_str, $count, "\n" . implode("\n", $paths ) ) );
					} else {
						do_action( 'qm/debug', sprintf( '[css concat] Bundle %s contains %d items: %s', $path_str, $count, "\n" . implode("\n", $paths ) ) );
					}

					$href = $siteurl . "/_static/??" . $path_str;
				} else {
					do_action( 'qm/debug', sprintf( '[css concat] Skipped: %s. Reason: lonely (nothing to concatenate with)', implode( $css ) ) );
